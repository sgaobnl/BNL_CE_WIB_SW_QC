...
def wib_dec(data, fembs=range(4), spy_num= 1, fastchk = False, cd0cd1sync=True): #data from one WIB  
    ...
    for sn in range(spy_num):
        ...
        if 1 in fembs:
            femb00 = np.transpose(dec_data[2][0])
            femb01 = np.transpose(dec_data[3][0])  
            tmts[2] = dec_data[2][1]
            tmts[3] = dec_data[3][1]   
            cd_tmts[2] = dec_data[2][3]
            cd_tmts[3] = dec_data[3][3]               
        if 2 in fembs:
            femb00 = np.transpose(dec_data[4][0])
            femb01 = np.transpose(dec_data[5][0])
            tmts[4] = dec_data[4][1]
            tmts[5] = dec_data[5][1]            
            cd_tmts[4] = dec_data[4][3]
            cd_tmts[5] = dec_data[5][3]   
        if 3 in fembs:
            femb00 = np.transpose(dec_data[6][0])
            femb01 = np.transpose(dec_data[7][0])            
            tmts[6] = dec_data[6][1]
            tmts[7] = dec_data[7][1]

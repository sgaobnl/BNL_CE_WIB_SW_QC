...
#        ax2.plot(chndata[npos-20:npos+20])
    ...
    for chn in chns:
        ...
        ax2.plot(np.array(wfs[chn][30:70]) - peds[chn],  marker='.')
    ...
    ampps = np.array(pkps) - np.array(peds)
    maxamp = round(np.max(ampps), 2)
    #minamp = round(np.min(ampps), 2)
    for i in range(0,128,16):
        ax3.vlines(i-0.5, 0, 17000, color='y')
        ax4.vlines(i-0.5, maxamp*0.90, maxamp, color='y')
    ...
    ax4.plot(chns, np.array(pkps) - np.array(peds), color='r', marker='.')
    #ax4.plot(chns, peds, color='b', marker='.')
    #ax4.plot(chns, np.array(pkns) - np.array(peds), color='m', marker='.')
    ax4.set_title("Pulse Response Distribution", fontsize=8)
    ax4.set_xlabel("Amplitude", fontsize=8)
    ax4.set_ylabel("CH number", fontsize=8)
    ax4.set_ylim((maxamp*0.90, maxamp*1.05))

#    a44.plot(chns,rmss, color='r', marker='.')
#    a44.set_title("Noise", fontsize=8)
#    a44.set_xlabel("ADC / bit", fontsize=8)
#    ax4.set_ylabel("CH number", fontsize=8)
    ax1.grid()
    ax2.grid()
...
#    fdir = froot + fsubdir + "/"
    ...
    if 0 in tms:
        ...
        for onekey in dkeys:
            if ("CHK" in onekey) or ("ASICDAC_CALI_CHK" in onekey):
            #if ("CHK" in onekey) or ("DIRECT_PLS_CHK" not in onekey) or ("ASICDAC_CALI_CHK" in onekey):
                cfgdata = data[onekey]
                fembs = cfgdata[0]
                rawdata = cfgdata[1]
                cfg_info = cfgdata[2]
...
#                    print (onekey)
                    ...
                    maxamp = round(np.max(ampps), 2)
                    for i in range(0,128,16):
                        ax1.vlines(i-0.5, 0, maxamp*1.05, color='y')


                    #ax2.plot(pkns, marker='.', label=onekey)
                if "SNC%d"%snc in onekey:
                    if "ASICDAC24" in onekey:
                        print (onekey)
                        ax2.plot(np.array(pkps)-np.array(peds), marker='.', label=onekey)
                        ampps = np.array(pkps) - np.array(peds)
                        maxamp = round(np.max(ampps), 2)
                        for i in range(0,128,16):
                            ax2.vlines(i-0.5, maxamp*0.90, maxamp*1.05, color='y')
                            ax2.set_ylim((maxamp*0.90, maxamp*1.05))
        ...
        for snc in [0, 1]:
            ...
            for onekey in dkeys:
                if "SNC%d"%snc in onekey:
                    ...
                    for i in range(0,128,16):
                        ax1.vlines(i-0.5, 0, maxamp*1.05, color='y')
                    for i in range(8,128,16):
                        ax1.vlines(i-0.5, 0, maxamp*1.05, color='b')

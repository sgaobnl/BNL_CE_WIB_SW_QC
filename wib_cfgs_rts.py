...
#        time.sleep(0.1)
    ...
    def femb_cur_chk(self, femb_id , bias_ilim=0.3, dc0_ilim=0.9,dc1_ilim=0.9, dc2_ilim=1.9):
        t0 = time.time()
        i = 0
        while True:
            init_ok = True
            ...
            if not init_ok:
                ...
                i = i + 1
                if i >= 3 : #~200ms
                    print ("\033[91m" + "FEMB/DAT power consumption @ (power on) is not right"+ "\033[0m")
...
#return to "idle" in case other FEMB runs FC 
    ...
    def femb_adac_cali(self, femb_id, phase0x07=[0,0,0,0,0,0,0,0]):
        for chip in range(8):
            ...
            ##duty 50 50
            #self.femb_i2c_wrchk(femb_id, chip_addr=3-(chip//4), reg_page=(chip%4+1), reg_addr=0x6, wrdata=0xfa        )
            #self.femb_i2c_wrchk(femb_id, chip_addr=3-(chip//4), reg_page=(chip%4+1), reg_addr=0x7, wrdata=phase0x07[chip])

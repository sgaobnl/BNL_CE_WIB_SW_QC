...
if 1:
    ...
    for chip in range(8):
        for chipchx in range(16):
            chx = chip*16 + chipchx
            wfdata = np.array(wfs[chx])-peds[chx]
            if chx == 0:
                pos = np.where(wfdata == np.max(wfdata))[0][0] 
            #print (pos)
            ax1 = chip%4
            ax2 = chip//4
            #axs[ax2, ax1].plot(wfdata[pos-20:pos+60], color="C%d"%((chx-chip*16)%10), marker='.', label="%d"%chx)
            if chipchx < 10:
                marker = '.'
            else:
                marker = 'x'
            ...
            #print (pos)
            if pos < 20:
                axs[ax2, ax1].plot(list(wfdata[period-20 +pos:]) + list(wfdata[0:pos+60]), color="C%d"%((chx-chip*16)%10), marker=marker, label="%d"%chx)
            elif pos < period-60:
                axs[ax2, ax1].plot(wfdata[pos-20:pos+60], color="C%d"%((chx-chip*16)%10), marker=marker, label="%d"%chx)
            else:
                axs[ax2, ax1].plot(list(wfdata[pos-20:]) + list(wfdata[0:60 + pos - period]) , color="C%d"%((chx-chip*16)%10), marker=marker, label="%d"%chx)
            #axs[ax2, ax1].plot(wfdata, color="C%d"%((chx-chip*16)%10), marker=marker, label="%d"%chx)
            axs[ax2, ax1].set_title(f"ASIC{chip}")
            axs[ax2, ax1].grid(True)
            axs[ax2, ax1].legend()
    ...
    for chip in range(8):
        for chipchx in range(16):
            ...
            if chipchx < 10:
                ...
                marker = 'x'
            axs[ax2, ax1].plot(wfdata[pos-6:pos+14], color="C%d"%((chx-chip*16)%10), marker=marker, label="%d"%chx)
            axs[ax2, ax1].set_title(f"ASIC{chip}")
            axs[ax2, ax1].grid(True)
            axs[ax2, ax1].legend()

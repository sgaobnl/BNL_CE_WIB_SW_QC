...
class RIGOL_PS_CTL:
    ...
    def on(self, channels=[1, 2, 3]):
        if type(channels) is not list:
            ...
            if (self.get_on_off(channels) != True):
                print(
                    "RigolDP832 Error --> Tried to turn on Channel {} of the Rigol DP832, but it didn't turn on".format(
                        channels))
    ...
    def off(self, channels=[1, 2, 3]):
        if type(channels) is not list:
            ...
            if (self.get_on_off(channels) != False):
                print(
                    "RigolDP832 Error --> Tried to turn off Channel {} of the Rigol DP832, but it didn't turn off".format(
                        channels))
    ...
    def set_channel(self, channel, voltage=None, current=None, v_limit=None, c_limit=None, vp=None, cp=None):
        ...
        if (v_limit):
            if ((v_limit > 0.01) and (v_limit < 33)):
                ...
                response = float(self.powerSupplyDevice.read().strip())
                if (response != v_limit):
                    print("RigolDP832 Error --> Voltage protection was set to {}, but response is {}".format(v_limit,
                                                                                                             response))
            else:
                print("RigolDP832 Error --> OverVoltage protection must be between 0.01 and 30 Volts, was {}".format(
                    v_limit))
        ...
        if (c_limit):
            if ((c_limit > 0.001) and (c_limit < 3.3)):
                ...
                response = float(self.powerSupplyDevice.read().strip())
                if (response != c_limit):
                    print("RigolDP832 Error --> Current protection was set to {}, but response is {}".format(c_limit,
                                                                                                             response))
            else:
                print("RigolDP832 Error --> OverCurrent protection must be between 0.001 and 3.3 Amps, was {}".format(
                    c_limit))
    ...
    def __init__(self):
        ...
        self.ADDR = u'USB0::0x1AB1::0x0E11::DP8C184550811::INSTR'
        self.gen = None

...
def monitor_power_rail_analysis(interface, datadir, reportdir, fembNo):
    ...
    for ifemb in range(len(vfembs)):
        ...
        for key in vkeys:
            if "GND" in key:
                value1 = [abs(int(mvold[key][0] * LSB * 1000))]
                mvvold[key] = ' '.join(value1)

            elif "HALF" in key:
                value2 = mvvold[key.replace("_HALF", "")] = [abs(int((mvold[key][0] - mvold["GND"][0]) * LSB * 2 * 1000))]
                mvvold[key] = ' '.join(value2)
            else:
                value3 = [abs(int((mvold[key][0] - mvold["GND"][0]) * LSB * 1000))]
                mvvold[key] = ' '.join(value3)

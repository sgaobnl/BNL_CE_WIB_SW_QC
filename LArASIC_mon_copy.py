...
def dat_fe_vbgrs():
    ...
    for fe in range(8):
        ...
        tempOutput = [fe, datas[fe]*AD_LSB]
        rawData.append(tempOutput)

    outputFile = "./tmp_data/measure_VBGR_through_VBGR_pin.bin"
...
def dat_fe_mons(mon_type=0, sg0=0, sg1=1, sgp=0):
    ...
    if mon_type == 1:
        ...
        outputFile = "./tmp_data/measure_temperature_through_monitoring_pin.bin"
        with open(outputFile, 'wb') as f:
            pickle.dump(rawData, f)
    ...
    if mon_type == 2:
        ...
        outputFile = "./tmp_data/measure_VBGR_through_monitoring_pin.bin"
        with open(outputFile, 'wb') as f:
            pickle.dump(rawData, f)
    ...
    if mon_type == 3:
        ...
        for dac in range(64):
            ...
            #for fe in [0]:
                ...
                tempOutput = [fe, dac, datas[fe]*AD_LSB]
                rawData.append(tempOutput)

        outputFile = "./tmp_data/measure_LArASIC_DAC_through_monitoring_pin_sg0_{}_sg1_{}_sgp_{}.bin".format(sg0, sg1, sgp)
        with open(outputFile, 'wb') as f:
            pickle.dump(rawData, f)
    ...
    if mon_type == 0:
        ...
        for chn in range(16):
            ...
            #for fe in [0]:
                ...
                tempOutput = [fe, chn, datas[fe]*AD_LSB]
                rawData.append(tempOutput)

        outputFile = "./tmp_data/measure_LArASIC_through_monitoring_pin_200mVBL.bin"
        with open(outputFile, 'wb') as f:
            pickle.dump(rawData, f)
        ...
        outputFile = "./tmp_data/measure_LArASIC_through_monitoring_pin_900mVBL.bin"
        with open(outputFile, 'wb') as f:
            pickle.dump(rawData, f)
...
dat_fe_mons(mon_type=3, sg0=0, sg1=0, sgp=1) #disable gain
dat_fe_mons(mon_type=3, sg0=0, sg1=0, sgp=0) #14mV/fC
dat_fe_mons(mon_type=3, sg0=1, sg1=0, sgp=0) #4.7
dat_fe_mons(mon_type=3, sg0=0, sg1=1, sgp=0) #7.8
dat_fe_mons(mon_type=3, sg0=1, sg1=1, sgp=0) #25mV/fC
